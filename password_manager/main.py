...
def verify_master_password(cipher):
    if not os.path.exists(CHECK_FILE):
        with open(CHECK_FILE, "wb") as file:
            file.write(cipher.encrypt(b"verified"))

        return True

    try:
        with open(CHECK_FILE, "rb") as file:
            encrypted_check = file.read()

        cipher.decrypt(encrypted_check)
        return True
...
def view_passwords(cipher):
    ...
    try:
        ...
        print("Theres are no passwords saved.")
    ...
    try:
        ...
        with open(PASSWORD_FILE, "wb") as file:
            ...
            for line in lines:
                ...
                if website_to_delete not in website:
                    file.write(line)
                else:
                    found = True
